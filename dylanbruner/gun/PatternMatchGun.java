...
public class PatternMatchGun extends Component {
    ...
    public Point2D.Double doPatternGun(ScannedRobotEvent e, double bulletPower) {
        ...
        for (int i = snapshots.size() - 1; i >= 0; i--) {
            if (pattern.size() < snapshotsToUse) {
                pattern.add(snapshots.get(i));
            } else {
                break;
            }
        }
        ...
        for (int i = 0; i < snapshots.size() - snapshotsToUse; i++) {
            ...
            if (closestPattern.size() == 0) {
                closestPattern = currentPattern;
            } else {
                // Get the difference between the current pattern and the pattern we are trying
                // to match
                double difference = getPatternDiffrence(currentPattern, pattern);

                // Get the difference between the closest pattern and the pattern we are trying
                // to match
                double closestDifference = getPatternDiffrence(closestPattern, pattern);

                // If the current pattern is closer to the pattern we are trying to match, set
                // it as the closest pattern
        ...
        if (difference > 18_000) {
            // System.out.println("[DEBUG] Pattern matching failed, difference: " +
            // difference);
            return estimateRobotLocation(e, bulletPower); // If pattern matching failed, use basic estimation math
        }

        // Get the enemy's current location
        Point2D.Double enemyLocation = MathUtils.project(alphabet.myLocation,
                alphabet.getHeadingRadians() + e.getBearingRadians(), e.getDistance());
        Point2D.Double predictedEnemyLocation = new Point2D.Double(enemyLocation.x, enemyLocation.y); // Make a copy of
                                                                                                      // the enemy's
                                                                                                      // location

        predictedEnemyLocation = predictLocationFromPattern(
                closestPattern.toArray(new EnemySnapshot[closestPattern.size()]), predictedEnemyLocation);

        // Limit the predicted enemy location to the field dimensions using Math.min and
        // Math.max
